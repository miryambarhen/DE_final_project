...
df_all_users = spark.read.format("com.mongodb.spark.sql.DefaultSource").option("uri",
                                                                               "mongodb://localhost:27017/stocks_db.users").schema(
    mongo_schema).load()

df_relevant_users_for_the_stock = df_all_users.join(df_realtime_prices,
                                                    df_realtime_prices['stock_ticker'] == df_all_users['stock_ticker'],
                                                    'inner')
